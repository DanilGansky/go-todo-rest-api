...
import (
	...
	"github.com/danikg/go-todo-rest-api/user/repository/mock"
)

func TestUserService_GetAll(t *testing.T) {
	userService := NewUserService(&mock.UserRepositoryMock{})
	users, err := userService.GetAll()
	assert.NoError(t, err)
	assert.NotEmpty(t, users)

	userService = NewUserService(&mock.UserRepositoryMock{GenerateErr: true})
	users, err = userService.GetAll()
	assert.Error(t, err)
